...
1.Unit-тесты на логику квиза
Проверка загрузки вопросов, выбора ответа, навигации между вопросами, подсчёта результатов и работы таймера.Тесты запускаются через Jest.
2.Перемешивание вопросов и вариантов.Каждый раз при загрузке вопросы и варианты случайным образом перемешиваются.
3.Режим обзора (review mode)После завершения теста можно включить просмотр всех вопросов с подсветкой правильных и неправильных ответов.
4.После окончания теста нельзя менять ответы.
5.Показ времени в мм:сс.Тест автоматически завершается при истечении времени.
6.Quiz и QuizEngine вынесены в отдельный файл (quiz.js), логика отделена от DOM.
